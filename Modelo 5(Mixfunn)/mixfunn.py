...
class MixFunn(nn.Module):
    ...
    def __init__(self,N_layers=1,N_neuro=2, second_order_function=False  ):
        super(MixFunn, self).__init__()
        self.N_layers = N_layers
        if N_layers == 1:
            self.layers = Mixfun(1, 1, second_order_function=second_order_function)
        if self.N_layers == 2:
            self.layers1 = Mixfun(1, N_neuro, second_order_function=second_order_function)
            self.layers2 = Mixfun(N_neuro, 1, second_order_function=second_order_function)
